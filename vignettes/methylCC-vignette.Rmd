...
which are based on methods related
to surrogate variable analysis or
...
Load the `compote` R package and other packages that we'll need 
later on. 
...
1. an `object` which is inherited from an
`eSet` such as an `RGChannelSet` from
the R package `minfi` or a `BSseq` object
from the R package `bsseq`. The `object`
can also be a data frame or matrix with observations
(e.g. CpGs) on the rows and samples as the columns.
...
To run the `compote::estimateCC()` function,
...
Here we are also specifying that we would like standard
error estimates to be included setting the
parameters `includeSE=TRUE` and `B` to a
number larger than 0. `B` represents the number
of bootstrapped samples to create to calculate the
standard errors of the cell composition estimates.
...
To see a plot of the cell composition estimates
```{r run-estimateCC-plots, fig.height=6, fig.width=8}
estimateCCPlot(est)
```
...
Then, we can compare the estimates to `compote::estimateCC()`. 
