...
`cell_counts()` function. To see summary information 
...
```{r compare-estimates}
df_minfi = gather(cbind("samples" = summary(est)$sample_names,
                        as.data.frame(est_minfi)),
                  celltype, est, -samples)

df_methylCC = gather(cbind("samples" = summary(est)$sample_names, 
                           cell_counts(est)),
                     celltype, est, -samples)
